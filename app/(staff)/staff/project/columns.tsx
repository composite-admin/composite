...
export const columns: ColumnDef<IProjectData>[] = [
  {
    ...
    cell: ({ row }) => {
      return (
        <Link href={`/staff/project/${row.original["id"]}`}>
...

  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Link
          href={`/staff/project/add-worker?name=${row.original["project_name"]}`}
        >
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <Link href={`/staff/project/${row.original["id"]}/add-material`}>
          <div className="cursor-pointer">
            <span className="font-semibold text-primaryLight-500 flex items-center hover:underline">
              <RiToolsFill />
              Add{" "}
